...
class NewsListViewModel @Inject constructor(
    ...
    fun fetchNewsBySources(sourceId: String) {
        if (checkInternetConnection()) {
            fetchNewsBySourcesByNetwork(sourceId)
        } else {
            fetchNewsBySourcesByDB(sourceId)
        }
    }
    ...
    fun fetchNewsByCountry(countryId: String) {
        if (checkInternetConnection()) {
            fetchNewsByCountryByNetwork(countryId)
        } else {
            fetchNewsByCountryByDB(countryId)
        }
    }
    ...
    fun fetchNewsByLanguage(languageId: String) {
        ...
        if (checkInternetConnection()) {
            ...
            fetchNewsByLanguageByDB(languageIdFirst.toString(), languageIdSecond.toString())
        }
    }
